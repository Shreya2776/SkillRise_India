...
const SkillTag = ({ name, onRemove }) => (
  <Badge variant="default" className="group">
    {name}
    {onRemove && (
      <button onClick={onRemove} className="ml-1 text-indigo-400/40 hover:text-white transition-colors">
  ...
  <div className="space-y-3">
    ...
    <div className="relative group">
       <input 
          ...
          className="w-full bg-white/[0.03] border border-white/5 rounded-[1.2rem] p-5 text-sm font-bold text-white outline-none focus:border-indigo-500/40 focus:bg-indigo-500/[0.02] focus:shadow-[0_0_20px_rgba(99,102,241,0.05)] transition-all placeholder:text-white/20"
...
const InterviewPage = () => {
  ...
  if (!isSetupComplete) {
    return (
      <div className="w-full h-full bg-[#06060a] flex flex-col items-center px-10 py-12 lg:px-20 overflow-y-auto scrollbar-hide">
        <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 py-12">
          ...
          <div className="lg:col-span-5 relative">
            ...
            <Card className="rounded-[3.5rem] p-12 space-y-12 relative z-10">
               ...
               <Button 
                ...
                variant="primary"
                className="w-full py-8 rounded-[2rem] font-black uppercase tracking-[0.6em] text-[11px] gap-6 group"
               >
  ...
  if (isSetupComplete && !isStarted) {
    return (
      <div className="w-full h-full bg-[#06060a] flex flex-col items-center justify-center p-12 text-center relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto space-y-16 pb-20">
                 ...
                 <div className="flex items-start gap-6 max-w-[90%]">
                    <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-white/20 shrink-0 border border-white/5 shadow-sm"><Bot size={22} /></div>
                     <div className="space-y-3">
                        ...
                        <Card className="rounded-3xl rounded-tl-none p-8 py-7">
          ...
          {currentQuestion && !isComplete && (
            <div className="flex items-start gap-6 animate-in fade-in slide-in-from-bottom-12 duration-1000">
               ...
               <div className="space-y-6 max-w-[85%]">
                  ...
                  <Card className="rounded-3xl rounded-tl-none p-8 py-7 relative group border-indigo-500/20">
          ...
          {loading && !currentQuestion && !isComplete && (
             <div className="flex items-start gap-6 opacity-30">
                <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-white/20 shrink-0 border border-white/5"><Bot size={22} /></div>
                 <Card className="rounded-2xl px-8 py-6 flex gap-2 items-center">
          ...
          {isComplete && finalReport && (
            <div className="py-20 animate-in slide-in-from-bottom-24 duration-1500 space-y-20">
               ...
               {/* Impact Card */}
               <Card className="rounded-[5rem] p-24 text-center relative overflow-hidden backdrop-blur-md">
                  ...
                  <h3 className="text-[11px] font-black text-white/20 uppercase tracking-[1em] text-center pt-10">NODE-BY-NODE ANALYSIS</h3>
                   <div className="space-y-10">
                     {finalReport.questionAnalysis?.map((node, idx) => (
                       <Card key={idx} className="rounded-[4rem] p-16 space-y-16 group hover:bg-white/[0.02]">
               ...
               <div className="flex flex-col sm:flex-row gap-8 pb-40">
                  <Button variant="primary" onClick={handleDownload} className="flex-1 py-12 rounded-[3.5rem] font-black uppercase tracking-[1em] text-xs gap-6"><Download size={22} /> EXPORT DOSSIER</Button>
                  <Button variant="secondary" onClick={() => navigate('/dashboard')} className="flex-1 py-12 rounded-[3.5rem] font-black uppercase tracking-[1em] text-xs gap-6 group"><ArrowLeft size={22} className="group-hover:-translate-x-2 transition-transform" /> COMMAND BASE</Button>
               </div>
      ...
      {!isComplete && isStarted && (
         <footer className="shrink-0 p-12 pt-0 bg-transparent z-50">
            <div className="max-w-5xl mx-auto group">
               <Card className="rounded-[3rem] p-5 pr-5 flex gap-5 shadow-4xl focus-within:border-indigo-500/30 hover:bg-[#12121a]">
                   <textarea 
